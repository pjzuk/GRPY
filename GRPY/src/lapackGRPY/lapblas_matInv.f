C April 9, 2013
C ALL NECESSARY LAPACK (1992) and BLAS (1992) SUBROUTINES
C NECESSARY TO COMPILE SUBROUTINE EIGEN(A,D,NN).
C ( sum of segments from 'lapblas_double_win.f' and 'lapblas_eigen_additional.f' )

C June 3, 2012
C      SUBROUTINE EIGEN(A,D,NN)
C      IMPLICIT NONE
C      INTEGER NN
C      REAL*8 A(NN,NN),D(NN)
C      REAL*8 E(NN-1),TAU(NN-1),WORK(2*NN)
C      INTEGER INFO
C
C      CALL DSYTRD('U', NN, A, NN, D, E, TAU, WORK, 2*NN, INFO )
C      IF(INFO.NE.0) WRITE(*,*) 'EIGEN ERROR'
C
C      CALL DORGTR('U', NN, A, NN, TAU, WORK, 2*NN, INFO )
C      IF(INFO.NE.0) WRITE(*,*) 'EIGEN ERROR'
C
C      CALL DSTEQR('V', NN, D, E, A, NN, WORK, INFO )
C      IF(INFO.NE.0) WRITE(*,*) 'EIGEN ERROR'
C
C      RETURN
C      END
...
************************************************************************
*
*     File of the REAL*8  Level-2 BLAS.
*     ===========================================
*
...
*          of the symmetric tridiagonal (or full) matrix.
*
